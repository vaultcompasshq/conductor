...
function managedHooksGuidance(
  ...
  relPath: string,
  detectedIn: 'package.json' | 'hook'
): string {
  const entry =
    manager === 'simple-git-hooks'
      ? 'the "pre-commit" entry under "simple-git-hooks"'
      : 'the "pre-commit" entry under "gitHooks"';
  const reinstall =
    manager === 'simple-git-hooks'
  ...
  const lead =
    detectedIn === 'package.json'
      ? `package.json declares ${manager}, which generates ${relPath} from that declaration and ` +
        'rewrites it on every install.'
      : `${relPath} was generated by ${manager} from a declaration in package.json, and is ` +
  ...
  if (!husky) {
    ...
    if (managedBy !== null) {
      conflicts.push({
        ...
        guidance: managedHooksGuidance(managedBy, rel, fromHook === null ? 'package.json' : 'hook'),
      });
      return { ...base, repoRoot: root, hookPath: executedHookPath, hookManager: managedBy };
    }
  }
